...
# ==========================================
# 3. Funciones de Pérdida (CONTROL INTERNO)
# ==========================================
def loss_nc():
...
def loss_c():
    ...
    # --- LA MAGIA DEL CONTROL INTERNO ---
    # Máscara que vale 1 solo si x está entre 0.4 y 0.6
    mask = ((x >= 0.4) & (x <= 0.6)).float()
    f = net_f(t) 
    
    # EDP Con control interno: 10*u_t - u_xx - u*(1-u) = mask * f(t)
    pde = torch.mean((10 * u_t - u_xx - u * (1.0 - u) - mask * f)**2)
    ...
    xr, tr = sample_right(N_bc)
    bc1 = torch.mean((net_u_c(torch.cat([xr, tr], 1)))**2) 
    
    xT, tT = sample_terminal(N_T)
    final = torch.mean((net_u_c(torch.cat([xT, tT], 1)))**2)
    
    reg = torch.mean((net_f(t))**2) # Se penaliza sobre los puntos interiores t
    
    return pde + 30*ic + 30*bc0 + 30*bc1 + 120*final + 1e-3*reg

# ==========================================
# 4. Entrenamiento
# ==========================================
print("Entrenando Caso 1 (No Lineal SIN Control Interno)...")
...
print("Entrenando Caso 2 (No Lineal CON Control Interno en [0.4, 0.6])...")
...
fig.suptitle("Evolución con Control Interno Distribuido en $\omega = [0.4, 0.6]$", fontsize=15, y=1.05)

for i, t in enumerate(times):
    u_nc = eval_u(net_u_nc, t)
    u_c = eval_u(net_u_c, t)
    
    # Sombreado de la zona de control
    axes[i].axvspan(0.4, 0.6, color='gray', alpha=0.2, label="Zona de control" if i==0 else "")
...
plt.title("Acción del Control Interno: Función $f(t)$ aprendida")
plt.xlabel("Tiempo (t)")
