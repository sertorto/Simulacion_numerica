...
# ===============================
# Ecuación: -u_xx + u = A * x  |  u(0)=0, u(1)=0
def solucion_exacta(x_np, A):
    sh1 = np.sinh(1.0)
    return A * (x_np - np.sinh(x_np) / sh1)
...
def compute_inverse_loss(model, u_data_target):
    ...
    # NUEVA FÍSICA: -u_xx + u = A_pred * x
    res_pde = -u_xx + u_pred - (model.A_pred * x_dom_tensor)
...
fig.suptitle('PIKAN Inversa: Descubriendo la función $F(x) = A \cdot x$', fontsize=18, fontweight='bold', y=0.96)

print("Iniciando Problema Inverso (F(x) = A*x) para A=1, A=2 y A=3...")
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % 30 == 0 or epoch == 1:
        ...
        axs[0, 2].set_ylim([-0.01, 0.33])
        
        for ax in axs[0, :]:
            ax.grid(True, ls='--'); ax.set_ylabel('$u(x)$'); ax.set_xlabel('$x$')
        axs[0, 0].legend(loc='upper left')
        
        # --- FILA 2: EVOLUCIÓN DE LA FUNCIÓN FUENTE F(x) = A*x ---
        axs[1, 0].plot(x_dom_np, x_dom_np * 1.0, 'k-', lw=3, alpha=0.2, label='F(x) Real')
        axs[1, 0].plot(x_dom_np, x_dom_np * a_pred1, 'r-', lw=2.5, label='F(x) Predicha')
        axs[1, 0].set_title(f"$F(x) = A\cdot x$ (Predicho A={a_pred1:.3f})")
        axs[1, 0].set_ylim([-0.05, 1.2])
        
        axs[1, 1].plot(x_dom_np, x_dom_np * 2.0, 'k-', lw=3, alpha=0.2)
        axs[1, 1].plot(x_dom_np, x_dom_np * a_pred2, 'g-', lw=2.5)
        axs[1, 1].set_title(f"$F(x) = A\cdot x$ (Predicho A={a_pred2:.3f})")
        axs[1, 1].set_ylim([-0.05, 2.2])
        
        axs[1, 2].plot(x_dom_np, x_dom_np * 3.0, 'k-', lw=3, alpha=0.2)
        axs[1, 2].plot(x_dom_np, x_dom_np * a_pred3, 'b-', lw=2.5)
        axs[1, 2].set_title(f"$F(x) = A\cdot x$ (Predicho A={a_pred3:.3f})")
        ...
        # ¡LA CORRECCIÓN ESTÁ AQUÍ! Añadimos .copy() para capturar fotogramas independientes
...
# 5. GUARDAR GIF
# ===============================
save_path = "/Users/molab/Documents/simnu/PIKAN_Inverso_x.gif"
