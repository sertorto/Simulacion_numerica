...
# ==========================================
T_final = 3.0
times = [0.0, 0.002, 0.01, 0.1, 0.5,1.0, 1.5, 3.0]
...
def u0_micro(x, epsilon):
    # Condición Inicial con amplitud alta y microestructura
    return 100*torch.sin(2.0 * np.pi * x / epsilon)**2
...
def ejecutar_experimento(epsilon):
    ...
    def loss_c():
        ...
        uxr = grad(ur, xr)
        f = net_f(tr)  # <-- control SIN microestructura
    ...
    # Evolución PDE con ejes libres para cada subplot
    fig, axes = plt.subplots(1, len(times), figsize=(20, 4), sharey=False)
    ...
    for i, t in enumerate(times):
        ...
        # ejes libres, ajustan automáticamente
        axes[i].autoscale(enable=True, axis='y')

    plt.tight_layout()
    plt.show()

    # Función de control (sin microestructura)
