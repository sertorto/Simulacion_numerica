...
x_train = linspace(-2*pi, 2*pi, 30);
y_train = sin(x_train);
%% ---------------- PESOS EN LA PERDIDA -------------------
p = ones(size(x_train));
p(1)   = 1;   % extremo izquierdo
p(end) = 5;   % extremo derecho
%% ---------------- FUNCION OBJETIVO ----------------------
f = @(x) sin(x);
%% ---------------- PARAMETROS DE LA RED ------------------
N = 8;                  % numero de neuronas ocultas
A  = @(x) tanh(x);
dA = @(x) 1 - tanh(x).^2;
%% ---------------- INICIALIZACION ------------------------
rng(1)
a = randn(1,N);         % pesos de salida
w = 0.5*randn(1,N);       % pesos de entrada
...
c = c - eta*dc;
%% --------- VISUALIZACION ----
if (epoch <= 100 && mod(epoch,100)==0) || ...
  (epoch > 100  && mod(epoch,500)==0) || ...
  epoch == epochs
